...
Live checklist updated: 2026-09-07.

Current software inventory and validation: [`DEVELOPMENT_STATUS.md`](./DEVELOPMENT_STATUS.md).
The checkboxes below remain full acceptance criteria, not a code-only completion
list. Evidence/clinical-review requirements are temporarily waived as development
blockers by the user; clinical, device, and public-release acceptance are not waived.

Implementation note (2026-09-07): the locally verifiable Phase 2 seams are implemented
and linked to automated proof in [`PHASE_2_EXECUTION_PLAN.md`](./PHASE_2_EXECUTION_PLAN.md).
Unchecked boxes that depend on dated clinical/editorial, accessibility/device,
performance, legal, or store artifacts remain acceptance gates; they are not claims that
the corresponding implementation is absent. The manual evidence record is
[`PHASE_2_ACCEPTANCE_MATRIX.md`](./PHASE_2_ACCEPTANCE_MATRIX.md).

Phase 0 and Phase 1 implementation slices exist, but their original device,
accessibility, and release-acceptance gates are not all proven. This file tracks the
active Phase 2 requirements; it does not imply that earlier acceptance evidence is
complete. The original implementation plan described Phase 2 as “Profile breadth and
emergency pathway,” but ADR 0002 moved the offline emergency shell—including unknown
...
features already implemented in Phase 1.

See [Phase 2 status and execution plan](./PHASE_2_EXECUTION_PLAN.md) for the
evidence-backed status of every deliverable and the recommended implementation order.
The durable implementation objective is recorded in [the Codex goal](./CODEX_GOAL.md),
and product/external blockers are tracked in
[the owner decision packet](./OWNER_DECISIONS.md).
...
Michael Mercer locked the recommended defaults on 2026-09-07. The complete decisions,
conservative behavior, and remaining external artifacts are recorded in
[`OWNER_DECISIONS.md`](./OWNER_DECISIONS.md).

- [x] **Evidence vocabulary and presentation:** final user-facing evidence-strength /
      literature-depth labels, when they appear, and how conflicting evidence is surfaced.
- [x] **Evidence assessment policy:** final required fields/thresholds for evidence basis,
      applicability, limitations, conflict flags, and review cadence.
- [x] **Medical/editorial governance:** named owners/reviewers, two-person review policy,
      accepted source standards, correction workflow, and urgent safety-update procedure.
- [x] **Emergency launch-market configuration:** launch countries/locales, emergency-call
      numbers/behavior, and the approved locale fallback policy. A worldwide emergency
      directory is not assumed.
- [x] **Public-release scope:** which Phase 2 content may remain clearly marked draft in
      development builds versus what must be reviewed before beta/public release.
- [x] **Testing-preview boundary:** exactly which preview metadata/copy is included in a
      profile before the later full reagent/testing workflow, without accidentally building
      that workflow in Phase 2.
- [x] **Performance budgets:** final measurable budgets for cold launch, profile render,
...
- [x] **Launch-market decision:** emergency calling behavior cannot be considered
...
- [x] Every unresolved decision that blocks Phase 2 sign-off is locked and documented.
- [x] External release gates are either satisfied or clearly distinguished from
