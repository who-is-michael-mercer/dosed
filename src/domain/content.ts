...
export const safetyClaimSchema = sourced.extend({
  id: stableIdSchema,
...
export const substanceSchema = sourced.extend({
  id: stableIdSchema,
  name: z.string().min(1),
  aliases: z.array(z.object({ text: z.string(), kind: z.string(), display: z.boolean() })),
  searchTerms: z.array(z.string()),
  categoryIds: z.array(stableIdSchema).min(1),
  visual: z.object({ symbol: z.string(), color: z.string() }),
  identity: z.string().min(1),
  review: reviewSchema,
  safetyClaims: z.array(safetyClaimSchema),
  doseReferences: z.array(z.unknown()).optional(),
  timelines: z.array(z.unknown()).optional(),
  effects: z.any().optional(),
  testing: z.any().optional(),
  helpSigns: z.array(z.string()).optional(),
